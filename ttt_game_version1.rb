...
class Player
  ...
  def self.set_computer_marker
    %w(O U P B A C).sample
  end
...
class TTTGame
  HUMAN_MARKER = Player.pick_a_marker
  COMPUTER_MARKER = Player.set_computer_marker
  ...
  def main_game_loop
    loop do
      ...
      winner_counter
      display_result
      break if @win_counter.values.take(2).max >= 5
      reset
      display_board
    end
  end
