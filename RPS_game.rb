...
class Player < Decisions
  ...
  def give_name
    ...
    loop do
      ...
      p_name = gets.chomp
      break unless p_name.empty?
    end
    self.name = p_name
  end
end

class Computer < Decisions
  def initialize
    give_name
  end
...
class RPSGame
  ...
  def display_current_results
    print "Scores: Player[#{@win_counter['player_win_counter']}] "
    print "Computer[#{@win_counter['computer_win_counter']}] "
